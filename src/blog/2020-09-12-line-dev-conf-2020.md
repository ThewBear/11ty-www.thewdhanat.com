...
สิ่งที่สนุกที่สุดในงานคงเป็นการเล่นเกมแต่ละท้าย Session เพื่อ[สะสมคะแนนรับของรางวัล](https://facebook.com/events/749022509005252/?post_id=762762727631230&view=permalink) โดยเกมมีทั้งรูปแบบที่เป็นตอบคำถามและหมุนวงล้อ

จะเห็นว่าในปีที่ผ่านมา LINE เปิดตัว API ต่าง ๆ เร็วเพิ่มมากขึ้น และมีคนนำไปใช้อย่างรวดเร็ว ตอนนี้ Use Case ต่าง ๆ ก็ยังเหลืออีกมากมายที่ยังรอการคิดค้น และพัฒนาต่อยอด
...
- เปิดตัว [LINE Developers Codelabs](codelab.line.me)
- hackth.line.me
...
ณ ตอนนี้มี [ex10 Chat Tool](https://ex10.tech) ที่เพียงนำไปไว้ตรงกลางระหว่าง LINE กับ Webhook เดิมของเรา ก็สามารถทำให้ใช้ Bot กับ Chat Mode พร้อมกันได้ นอกจากนี้ถ้า Admin ตอบภายใน 30 วินาที ระบบจะใช้ Reply Token ให้แทน และ Admin ยังสามารถส่งรูป และไฟล์ จากกล่องแชทได้เลยอีกด้วย
...
OA Plus คือ Platform ที่สามารถขยายได้ (Extensible) โดยเป็นระบบที่อยู่เบื้องหลัง LINE My Shop ซึ่งพัฒนาโดยทีมประเทศไทย และกำลังขยายจะไปทั่วโลก สิ่งที่น่าสนใจคือระบบนี้ รองรับ Bot + Chat Mode พร้อมกัน
...
1. Service Mesh is Hard (But canbe really helpful)
   ...
   - ข้อดี
     - Smart proxy
     - Decoupling RBAC
   - แต่ใช้แล้ว Upgrade ยาก จึงควรนับ Major upgrade
1. (CORE) DNS is the Heart of Many Things
   - แต่ถ้าพังแล้วหายาก
   - OA Plus ใช้ CoreDNS + Auto path + Node local cache
1. Kafka VS. Rabbit MQ
   ...
   - ระวัง Kafka
     - poison message เพราะเอาออกยาก
     - Kafka offset (Zookeeper) มีหมดอายุ
     - ข้อความหาย เช็คที่ Publisher library
...
1. Prepare for integration
   เมื่อมี Features มากขึ้น ก็ต้องมีคนมากขึ้น จึงต้องเตรียมพร้อมไว้ก่อน อาจใช้ Microservice ช่วย
1. Leverage CDN
